(function (global) {
    if (CKEDITOR.plugins.get('word_cloud')) {
        return;
    }
    ...
    global.CKEDITOR.on('instanceCreated', function (e) {
        $('#' + e.editor.name).parent().find('.ez-richtext-tools').after('<div class="mugo_word_cloud"></div>');
        const language = $('meta[name="LanguageCode"]').attr('content').split('-')[0];
        e.editor.on('change', function (event) {
            var wordsArray = e.editor.getData()
                    .replace(/<\/?[^>]+(>|$)/g, " ")
                    .toLowerCase()
                    .replace(/[`~!@#$%^&*()_|+\-=?;:'"“”,–.<>\{\}\[\]\\\/]/gi,' ')
                    .replace(/nbsp/gi, ' ')
                    .split(' ')
                    .filter(function (el) {
                        return el != '' && !stopWords[language].includes(el); 
                    });

            var words = wordsArray.reduce(function ( stats, word ) {
            ...
            for (var word in words) {
                wordsResult += '<span style="font-size:' + ( 12 + words[word]*5 ) + 'px;">' + word +  '</span> ';
            }
            $('#' + e.editor.name).parent().find('.mugo_word_cloud').html(wordsResult);
        });
    });
})(window);
